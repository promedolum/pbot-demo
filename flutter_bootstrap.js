...
// Renderer overrides are opt-in and exist for controlled compatibility and
// performance comparisons. Normal public URLs use the build's default.
const renderer = new URLSearchParams(window.location.search).get(
  'payvium_renderer',
);
const rendererExists = _flutter.buildConfig.builds.some(
  (build) => build.renderer === renderer,
);
const config = rendererExists &&
